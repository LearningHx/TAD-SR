...
from utils.util_image import spatial_padding
from utils.util_net import compute_hinge_loss

criterion_GAN = th.nn.BCEWithLogitsLoss().to('cuda:0')
...
class GaussianDiffusion:
    ...
    def weight_calculate(self, t):
        weight = torch.tensor((1 - self.kappa ** 2 * self.alpha)[t.detach().cpu()], device='cuda')
        return weight
    ...
    def p_mean_variance(
        ...
        model_mean, _, _ = self.q_posterior_mean_variance(  # �ㄦā����杈���x0璁＄��x_t_1������
    ...
    def p_sample_loop(
        ...
        """
        final = None
        for sample in self.p_sample_loop_progressive(       # �� LR + noise ����HR
            y,
            model,
            ...
            final = sample["sample"]    # ����HR�惧����latent code
        return self.decode_first_stage(final, first_stage_model)    # 灏�HR�� latent code 缂���瑙ｇ����HR�惧��
    ...
    def p_sample_loop_progressive(
        ...
        z_y = self.encode_first_stage(y, first_stage_model, up_sample=True)     # 灏�杈��ョ��LR宓��ュ�版��ㄧ┖��
        ...
        z_sample = self.prior_sample(z_y, noise)    # z_sample  = LR + noise锛�灏�杈��ラ��杩�������prior distribution
    ...
    def training_losses(
            self, model_stu, model_tea, model_dis, model_fea, x_start, y, t, t_tea,
            first_stage_model=None,
            model_kwargs=None,
            noise=None,
            optimizer_D=None,
        ...
        z_y = self.encode_first_stage(y, first_stage_model, up_sample=True)  # 灏�LR�惧��杩�琛�缂���
        z_start = self.encode_first_stage(x_start, first_stage_model, up_sample=False)  # 灏�HR�惧��杩�琛�缂���锛��垮�戒�琚���缂╁�颁�LR�惧��涓���

        if noise is None:
            noise = th.randn_like(z_start)

        # Forward Process锛��规�����㈤���哄���风��t, ����瀛���妯″��瀵瑰�璇ユ�跺�荤��x_state
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            ...
            # 瀛���妯″��: �规��x_t, 杈��轰及璁＄��x_t_1
            model_output_stu = model_stu(self._scale_input(z_t, t), t, **model_kwargs)  # x0
            #x0_stu = self.decode_first_stage(model_output_stu, first_stage_model=first_stage_model)
            # fea_list_stu, _ = model_fea(spatial_padding(model_output_stu, 14))
            _, cls_gt = model_fea(F.interpolate(z_start, size=70, mode='area'))
            #_, cls_y = model_fea(spatial_padding(z_y, 14))

            # Forward Process锛��规�����㈤���哄���风��t_tea, ������甯�妯″��瀵瑰�璇ユ�跺�荤��x_state
            z_t_tea_list = []
            t_tea_ = torch.split(t_tea, dim=0, split_size_or_sections=x_start.size(0))
            for i in t_tea_:
                z_t_tea_list.append(self.q_sample(model_output_stu, z_y, i, noise=noise))
            ...
            with th.no_grad():
                ...
                for i in z_t_tea_list:
                    model_output_tea = model_tea(self._scale_input(i.detach(), t_tea_[counter]), t_tea_[counter], **model_kwargs)
                    x0_tea_list.append(model_output_tea)
                    counter += 1

            z_start_ = torch.tensor(z_start, requires_grad=True, device='cuda')

            pred_fake = model_dis(model_output_stu.detach(), cls_gt.detach())
            pred_real = model_dis(z_start_, cls_gt.detach())

            # 璁＄���ゅ���ㄦ��澶�, �ゅ���ㄦ�搴�娓���, 杩�琛�����浼���, �存�板����
            r1_lambda = 0.0000
            loss_D = compute_hinge_loss(pred_real, pred_fake, z_start_, r1_lambda)
            ...
            # 璁＄�������ㄦ��澶
            pred_fake = model_dis(model_output_stu, cls_gt.detach())
            # gan_loss_list = []
            # for i in range(0, len(pred_fake)):
            #     gan_loss_list.append(-torch.mean(pred_fake[i]))
            terms["gan_loss"] = (-pred_fake).mean()

            # 涓���timestep������
            weight = self.weight_calculate(t_tea)
            weight_ = torch.split(weight, split_size_or_sections=x_start.size(0))

            # ��甯�妯″��涓�瀛���妯″��涔��寸���搁���澶�
            counter = 0
            dis_loss = []
            for i in x0_tea_list:
                dis_loss.append(mean_flat((i - model_output_stu)**2) * weight_[counter])
                counter += 1
            terms["dis_mse"] = torch.stack(dis_loss, dim=-1).mean()

            #terms["gt_mse"] = ((z_start - model_output_stu)**2).mean()

            # total loss
            terms["loss"] = 0.004*terms["gan_loss"] + terms["dis_mse"] #+ 0.2*terms["gt_mse"]

            print('D_loss:', loss_D.item(), 'G_loss:', terms["gan_loss"].item(), 'dis_loss:', terms["dis_mse"].item(), '\n\n')

        return terms, model_output_stu#, z_t, pred_zstart  # 杩���锛�璇�宸��硷�寰��扮��涓���x_state锛��规��涓���x_state棰�娴���x0
