...
class PairedCaptionDataset(data.Dataset):
    def __init__(
        ...
        for root_folder in root_folders:
            ...
            self.tch_list += glob.glob(os.path.join(tch_path,'*.png'))


        assert len(self.lr_list) == len(self.gt_list)
        assert len(self.lr_list) == len(self.tag_path_list)
        assert len(self.lr_list) == len(self.tch_list)
    ...
    def __getitem__(self, index):
        ...
        tch_path = self.tch_list[index]
        tch_img = Image.open(tch_path).convert('RGB')
        tch_img = self.img_preproc(tch_img)
        ...
        example["tch_out"] = tch_img.squeeze(0) * 2.0 - 1.0
