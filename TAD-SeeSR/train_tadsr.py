...
from ADD.models.discriminator import ProjectedDiscriminator,Time_Aware_Discriminator
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--root_folders",  type=str, default='./preset/datasets/train_realsr')
    ...
    parser.add_argument("--ram_input_different", type=bool, default=False)

    parser.add_argument("--GAN_weight", type=float, default=0.2)
    parser.add_argument("--hsd_weight", type=float, default=1)
...
# 实例化判别器头
model_dis =Time_Aware_Discriminator(c_dim=320,embed_dim=320).train()
...
controlnet, unet, optimizer, train_dataloader, lr_scheduler, controlnet_stu, unet_stu, model_dis, optimizer_D = accelerator.prepare(
    controlnet, unet, optimizer, train_dataloader, lr_scheduler,
    controlnet_stu, unet_stu,  model_dis, optimizer_D,
)
...
for epoch in range(first_epoch, args.num_train_epochs):
    for step, batch in enumerate(train_dataloader): 
        ...
        with accelerator.accumulate(controlnet), accelerator.accumulate(unet), accelerator.accumulate(controlnet_stu), accelerator.accumulate(unet_stu):
            ...
            tch_out = batch["tch_out"].to(accelerator.device, dtype=weight_dtype)   

            vae.to(accelerator.device)
            # Convert images to latent space
            latents = vae.encode(pixel_values).latent_dist.sample() 
            latents = latents * vae.config.scaling_factor
            tch_latents = vae.encode(tch_out).latent_dist.sample()
            tch_latents = tch_latents * vae.config.scaling_factor
            vae = vae.cpu()                                          # Move it to the cpu to save memory
            ...
            text_encoder = text_encoder.cpu()         # Move it to the cpu to save memory
            torch.cuda.empty_cache()

            # extract soft semantic label
            RAM = RAM.to(accelerator.device)
            with torch.no_grad():
            ...
            RAM = RAM.cpu()                 # Move it to the cpu to save memory
            torch.cuda.empty_cache()

            ###################################
            ###---Generate images through one step sampling---### 
            ###################################
            ...
            timesteps_dis_list = list(range(20,900,20))
            select_time = random.choice(timesteps_dis_list)
            timesteps_dis = torch.full( (bsz,),select_time,device=latents.device)
            timesteps_dis = timesteps_dis.long()

            noise_dis = torch.randn_like(latents)
            z_t_stu = (noise_scheduler.add_noise(z0_stu, noise_dis, timesteps_dis))  
            z_t_gt = (noise_scheduler.add_noise(latents, noise_dis, timesteps_dis))
            ...
            with torch.no_grad():                          
                down_block_res_samples_tea, mid_block_res_sample_tea, repre_tea, features_tea = controlnet(
                    ...
                    timesteps_dis,
                    encoder_hidden_states=encoder_hidden_states,
                    controlnet_cond=controlnet_image if  args.ram_input_different else pixel_values / 2 + 0.5,
                    return_dict=False,
                    image_encoder_hidden_states=ram_encoder_hidden_states if args.ram_input_different else ram_encoder_hidden_states_tea,
                )
            ...
            down_block_res_samples_fake, mid_block_res_sample_fake, repre_tea, features_tea = controlnet(
                z_t_stu,
                timesteps_dis,
                encoder_hidden_states=encoder_hidden_states,
                controlnet_cond = controlnet_image if  args.ram_input_different else pixel_values / 2 + 0.5,
            ...
            fake_fea = unet(
                ...
                encoder_hidden_states=encoder_hidden_states,
                down_block_additional_residuals=[
                    sample.to(dtype=weight_dtype) for sample in down_block_res_samples_fake
                ],
                mid_block_additional_residual=mid_block_res_sample_fake.to(dtype=weight_dtype),
                image_encoder_hidden_states=ram_encoder_hidden_states if args.ram_input_different else ram_encoder_hidden_states_tea,
                only_fea=True,
            )
            ...
            # compute the adversarial loss & update parameters
            pred_real= model_dis(real_fea,timesteps_dis) 
            pred_fake = model_dis(fake_fea,timesteps_dis,detach=True)
            ...
            with torch.no_grad():   
                timesteps_hsd_list = list(range(20,900,20))
                select_time = random.choice(timesteps_hsd_list)
                timesteps_hsd = torch.full( (bsz,),select_time,device=latents.device)
                timesteps_hsd = timesteps_hsd.long()

                noise_hsd = torch.randn_like(latents)
                z_t_stu_hsd = (noise_scheduler.add_noise(z0_stu, noise_hsd, timesteps_hsd))  
                if args.target=="GT":
                    z_t_gt_hsd = (noise_scheduler.add_noise(latents, noise_hsd, timesteps_hsd))
                else:
                    z_t_gt_hsd = (noise_scheduler.add_noise(tch_latents, noise_hsd, timesteps_hsd))
            ...
            pred_fake= model_dis(fake_fea,timesteps_dis)
            pred_fake = torch.cat(pred_fake, dim=1)
            gan_loss = -torch.mean(pred_fake) * args.GAN_weight

            # compute the hsd loss
            hsd_loss = args.hsd_weight * mean_flat((model_pred_tea_stu - model_pred_gt) * z0_stu)

            # compute the regression loss
            if args.target=="GT":
                mse_loss = args.mse_weight * mean_flat((z0_stu - latents.detach())**2)
            else:
                mse_loss = args.mse_weight * mean_flat((z0_stu - tch_latents.detach())**2)

            loss = hsd_loss + gan_loss + mse_loss
        ...
        logs = {"D_loss": loss_D.detach().item(), "G_loss": gan_loss.detach().item(),"hsd_loss": hsd_loss.detach().item(), "mse_loss": mse_loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0]}
        progress_bar.set_postfix(**logs)
        accelerator.log(logs, step=global_step)
        with open("./logs/loss.txt","a") as f:
            f.write(f'time:{timesteps_hsd.item()},D_loss: {loss_D.detach().item()}, G_loss{gan_loss.detach().item()},hsd_loss:{hsd_loss.detach().item()}, mse_loss:{mse_loss.detach().item()}\n')
