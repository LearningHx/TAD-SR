...
from pipelines.pipeline_tadsr import StableDiffusionControlNetPipeline
...
def load_tadsr_pipeline(args, accelerator, enable_xformers_memory_efficient_attention):
    ...
    unet = UNet2DConditionModel.from_pretrained(args.tadsr_model_path, subfolder="unet")
    controlnet = ControlNetModel.from_pretrained(args.tadsr_model_path, subfolder="controlnet", low_cpu_mem_usage=False)
...
def main(args, enable_xformers_memory_efficient_attention=True,):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("TAD-SR")

    pipeline = load_tadsr_pipeline(args, accelerator, enable_xformers_memory_efficient_attention)
        ...
        for image_idx, image_name in enumerate(image_names[:]):
            ...
            for sample_idx in range(args.sample_times):  
                with torch.autocast("cuda"):
                    ...
                    end_time = time.time()
                    #print('iter_time_'+str(args.num_inference_steps)+'step',(end_time - start_time))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tadsr_model_path", type=str, default='./preset/tadsr')
    ...
    parser.add_argument("--image_path", type=str, default='./preset/datasets/RealSR_CenterCrop-20240520T083645Z-001/RealSR_CenterCrop/test_LR')
    parser.add_argument("--output_dir", type=str, default='./output/realsr_crop_step1_2')
