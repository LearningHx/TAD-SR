...
class UNetModelSwin(nn.Module):
    ...
    def forward(self, x, timesteps, lq=None,only_fea = False):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        h = self.middle_block(h, emb)
        if only_fea:
            fea.extend([hs[3],hs[6],hs[9],hs[11],h])
            return fea
        else:
            for module in self.output_blocks:
                h = th.cat([h, hs.pop()], dim=1)
...
class ProjectedDiscriminator(nn.Module):
    def __init__(self, c_dim: int, embed_dim: int):
        ...
        heads += [str(2), DiscHead(self.embed_dim*2, c_dim)],
        heads += [str(3), DiscHead(self.embed_dim*4, c_dim)],
        heads += [str(4), DiscHead(self.embed_dim*4, c_dim)],
    ...
    def forward(self, x: th.Tensor,t,detach=False) -> th.Tensor:
        ...
        for k, head in self.heads.items():
            ...
            h = out_rest(h)
            input = th.flatten(h,start_dim=2)
