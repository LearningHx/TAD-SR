...
class GaussianDiffusion:
    ...
    def prior_sample_intermediate(self, y,x_stu,noise_hsd,t_hsd):
        """
        Generate samples from the prior distribution, i.e., q(x_T|x_0) ~= N(x_T|y, ~)
        ...
        :param noise: the [N x C x ...] tensor of degraded inputs.
        """
        if noise_hsd is None:
            noise_hsd = th.randn_like(y)
        if t_hsd is None:
            t_hsd = th.tensor([self.num_timesteps-1,]*y.shape(0),device=y.device).long()

        return x_stu + _extract_into_tensor(self.etas, t_hsd, y.shape) * (y-x_stu) + self.kappa*_extract_into_tensor(self.sqrt_etas,t_hsd,y.shape) * noise_hsd

    def training_losses_distill(
            self, model, teacher_model, x_start, y,z_y,stu_out,t,t_dis,stu_fea=None,
            first_stage_model=None,
            model_kwargs=None,
            noise=None, distill_ddpm=False,use_score_distil=False,distill_type='SDS',score_distill_target='teacher',uniform_sampling=True,hsd_weight=1,discriminator_model=None,GAN_weight=1,num_iter=None,
            ):
        """
        ...
        if True:
            ...
            if distill_ddpm:
                ...
                z_start_teacher = self.ddim_sample_loop(y, teacher_model, noise, first_stage_model, clip_denoised=True if first_stage_model is None else False, apply_decoder=False, model_kwargs=model_kwargs,tt=t)["sample"]
            ...
            if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
                ...
                #    weights = 1
                    ...
                    weights = 1 
                terms[loss_type] = terms[loss_type] * weights
        ...
        if use_score_distil:
            if distill_type == 'HSD':
                with th.inference_mode():
                    if uniform_sampling:
                        t_hsd = th.randint(0,self.num_timesteps-1,size=(model_output.size(0),),device=model_output.device)
                    else:
                        t_hsd = th.randint(0,3,size=(model_output.size(0),),device=model_output.device)
                        #int(self.num_timesteps/5)

                    noise_hsd = th.rand_like(model_output)
                    x_stu_hsd = self.prior_sample_intermediate(z_y,model_output,noise_hsd,t_hsd)
                    if score_distill_target =='teacher':
                        x_tch_hsd = self.prior_sample_intermediate(z_y,z_start_teacher,noise_hsd,t_hsd)
                    else:
                        x_tch_hsd = self.prior_sample_intermediate(z_y,self.encode_first_stage(x_start, first_stage_model, up_sample=False) ,noise_hsd,t_hsd)
                    tch_predict_x0 = self.ddim_sample(teacher_model,self._scale_input(x_tch_hsd,t_hsd),z_y,t_hsd,clip_denoised=True if first_stage_model is None else False, denoised_fn=None, model_kwargs=model_kwargs,ddim_eta = 0.0)["pred_xstart"]
                    stu_predict_x0 = self.ddim_sample(teacher_model,self._scale_input(x_stu_hsd,t_hsd),z_y,t_hsd,clip_denoised=True if first_stage_model is None else False, denoised_fn=None, model_kwargs=model_kwargs,ddim_eta = 0.0)["pred_xstart"]
                    tch_predict_noise = (x_tch_hsd-(tch_predict_x0+_extract_into_tensor(self.etas,t_hsd,tch_predict_x0.shape)*(z_y-tch_predict_x0)))/(_extract_into_tensor(self.sqrt_etas,t_hsd,tch_predict_x0.shape)*self.kappa)
                    stu_predict_noise = (x_stu_hsd-(stu_predict_x0+_extract_into_tensor(self.etas,t_hsd,stu_predict_x0.shape)*(z_y-stu_predict_x0)))/(_extract_into_tensor(self.sqrt_etas,t_hsd,stu_predict_x0.shape)*self.kappa)
                    grad_z = stu_predict_noise - tch_predict_noise
                    assert th.isfinite(grad_z).all()
                    grad_z = th.nan_to_num(grad_z.detach(),0.0, 0.0, 0.0)
                hsd_loss = grad_z.clone()*model_output
                del grad_z
                terms[loss_type+"_hsd"] = mean_flat(hsd_loss)*hsd_weight 
                terms["loss"]+=terms[loss_type+"_hsd"]
            else:                                                              #SDS
                with th.inference_mode():
                    if uniform_sampling:
                        t_sds = th.randint(0,self.num_timesteps-1,size=(model_output.size(0),),device=model_output.device)
                    else:
                        t_sds = th.randint(0,3,size=(model_output.size(0),),device=model_output.device)

                    noise_sds = th.rand_like(model_output)
                    x_stu_sds = self.prior_sample_intermediate(z_y,model_output,noise_sds,t_sds)
                    tch_predict_x0 = self.ddim_sample(teacher_model,self._scale_input(x_stu_sds,t_sds),z_y,t_sds,clip_denoised=True if first_stage_model is None else False, denoised_fn=None, model_kwargs=model_kwargs,ddim_eta = 0.0)["pred_xstart"]
                    tch_predict_noise = (x_stu_sds-(tch_predict_x0+_extract_into_tensor(self.etas,t_sds,tch_predict_x0.shape)*(z_y-tch_predict_x0)))/(_extract_into_tensor(self.sqrt_etas,t_sds,tch_predict_x0.shape)*self.kappa)
                    grad_z = tch_predict_noise - noise_sds 
                    assert th.isfinite(grad_z).all()
                    grad_z = th.nan_to_num(grad_z.detach(),0.0, 0.0, 0.0)
                sds_loss = grad_z.clone()*model_output
                del grad_z
                terms[loss_type+"_sds"] = mean_flat(sds_loss)*hsd_weight  
                terms["loss"]+=terms[loss_type+"_sds"]
            
            
        if  discriminator_model!=None:
            pred_fake = discriminator_model(stu_fea,t_dis)
            pred_fake = th.cat(pred_fake, dim=1)
            terms["GAN_loss"] = -th.mean(pred_fake,dim=-1)*GAN_weight
            terms["loss"]+=terms["GAN_loss"]

        return terms, z_t, pred_zstart

    def training_losses_D(
            self, tea_model,x_start, z_y,stu_out, t,
            first_stage_model=None,
            model_kwargs=None,
        ...
        noise_dis = th.rand_like(z_start_gt)
        t_dis = th.randint(0,self.num_timesteps-1,size=(z_start_gt.size(0),),device=z_start_gt.device)
        noisy_gt_dis = self.prior_sample_intermediate(z_y,z_start_gt,noise_dis,t_dis)
        noisy_stu_dis = self.prior_sample_intermediate(z_y,stu_out,noise_dis,t_dis)
        with th.no_grad():
            real_fea = tea_model(self._scale_input(noisy_gt_dis, t_dis), t_dis,only_fea=True, **model_kwargs)
        fake_fea = tea_model(self._scale_input(noisy_stu_dis, t_dis), t_dis,only_fea=True, **model_kwargs)
        pred_real = discriminator_model(real_fea,t_dis)
        pred_fake = discriminator_model(fake_fea,t_dis,detach=True)
        pred_fake = th.cat(pred_fake,dim=1)
        pred_real = th.cat(pred_real, dim=1)
        loss_real = th.mean(th.relu(1.0 - pred_real),dim=-1)
        loss_fake = th.mean(th.relu(1.0 + pred_fake),dim=-1)
        terms['GAN_loss_D'] = loss_real + loss_fake
        return terms,fake_fea,t_dis
