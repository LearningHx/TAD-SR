...
from models.unet import ProjectedDiscriminator
...
class TrainerBase:
    ...
    def setup_optimizaton(self):
        ...
        if self.use_GAN_loss:
            self.optimizer_D = torch.optim.AdamW(self.discriminator_model.parameters(),
                                           lr=self.configs.train.lr,
                                           weight_decay=self.configs.train.weight_decay)
...
class TrainerDistillDifIR(TrainerDifIR):
    def __init__(self, configs):
        ...
        self.use_score_distil = configs.train.get("use_score_distil")
        self.distill_type = configs.train.get("distill_type")
        self.use_fixed_time = configs.train.get("use_fixed_time")
        self.score_distill_target = configs.train.get("score_distill_target")
        self.uniform_sampling = configs.train.get("uniform_sampling")
        self.hsd_weight = configs.train.get("hsd_weight")
        self.use_GAN_loss = configs.train.get("use_GAN_loss")
        self.GAN_weight = configs.train.get("GAN_weight")
    ...
    def build_model(self):
        ...
        if self.use_score_distil and self.rank == 0:
            # assert not self.learn_xT
            self.logger.info(f"[INFO]: Optimizing the model using score distillation")

        if self.distill_type and self.rank == 0:
            # assert not self.learn_xT
            self.logger.info(f"[INFO]: score distillation technology: {self.distill_type}")

        if self.score_distill_target and self.rank == 0:
            self.logger.info(f"[INFO]: score distillation target: {self.score_distill_target},  which is only for the ablation study") 
            
            
        if self.uniform_sampling and self.rank == 0:
            self.logger.info(f"[INFO]: score distillation with uniform sampling")
        
        if self.use_GAN_loss and self.rank == 0:
            self.logger.info(f"[INFO]: Training the model through adversarial learning")

        if self.hsd_weight and self.rank == 0:
            self.logger.info(f"[INFO]: HSD loss weight: {self.hsd_weight}")

        if self.GAN_weight and self.rank == 0:
            self.logger.info(f"[INFO]: GAN loss weight: {self.GAN_weight}")
        ...
        if self.use_GAN_loss:
            discriminator_model = ProjectedDiscriminator(c_dim=64,embed_dim=160).train()
            if self.num_gpus > 1:
                self.discriminator_model = DDP(discriminator_model.cuda(), device_ids=[self.rank,], broadcast_buffers=False)  # wrap the network
            else:
                self.discriminator_model = discriminator_model.cuda()
    ...
    def training_step(self, data):
        ...
        if self.use_fixed_time:
            ...
            #print('fixed start time')
        else:
            time_steps = [3,7,11,14]
            tt = torch.ones_like(tt) * (time_steps[torch.randint(0,len(time_steps),size=(1,))])

        latent_downsamping_sf = 2**(len(self.configs.autoencoder.params.ddconfig.ch_mult) - 1)
        latent_resolution = micro_data['gt'].shape[-1] // latent_downsamping_sf

        model_kwargs={'lq':micro_data['lq'],} if self.configs.model.params.cond_lq else None
        z_y = self.base_diffusion.encode_first_stage(micro_data['lq'], self.autoencoder, up_sample=True) # TODO can be eliminated to speed up, since z_y is already obtained in self.ddim_sample_loop/p_sample_loop
        noise = torch.randn(
                size=z_y.size(),
                device=z_y.device,
                )
        z_t = self.base_diffusion.prior_sample(z_y, noise)
        model_output = self.model(self.base_diffusion._scale_input(z_t, tt), tt, **model_kwargs)
        self.optimizer_D.zero_grad()
        computer_losses_D = functools.partial(
            self.base_diffusion.training_losses_D,
            self.teacher_model,
            ...
            first_stage_model=self.autoencoder,
            model_kwargs=model_kwargs,
            discriminator_model = self.discriminator_model if self.use_GAN_loss else None,
        )

        if self.configs.train.use_fp16:
            with amp.autocast():
                if last_batch or self.num_gpus <= 1:
                    losses_D,stu_fea,t_dis = computer_losses_D()
                else:
                    with self.model.no_sync():
                        losses_D,stu_fea,t_dis = computer_losses_D()
                loss_D = losses_D["GAN_loss_D"] 
            scaler.scale(loss).backward()
        else:
            if last_batch or self.num_gpus <= 1:
                losses_D,stu_fea,t_dis = computer_losses_D()
                ...
                with self.model.no_sync():
                    losses_D,stu_fea,t_dis = computer_losses_D()
            loss_D = losses_D["GAN_loss_D"].mean()
            loss_D.backward()
        ...
        compute_losses = functools.partial(
            ...
            use_score_distil = self.use_score_distil,
            distill_type = self.distill_type,
            uniform_sampling =self.uniform_sampling,
            score_distill_target =self.score_distill_target,
            hsd_weight = self.hsd_weight,
            discriminator_model = self.discriminator_model if self.use_GAN_loss else None,
            GAN_weight = self.GAN_weight,
            num_iter = self.current_iters
        ...
        # make logging
        self.log_step_train(losses,losses_D, tt*0, micro_data, z_t, z0_pred, last_batch)
    ...
    def log_step_train(self, loss,loss_D, tt, batch, z_t, z0_pred, flag=False, phase='train'):
        '''
        param loss: a dict recording the loss informations
        param tt: 1-D tensor, time steps
        '''
        if self.rank == 0:
            ...
            if (self.current_iters % self.configs.train.log_freq[0] == 0 or self.current_iters == 1) and flag:
                ...
                for key, val in self.loss_mean.items():
                    log_str += f'{key}:{torch.abs(val[0]).item():.2e} '

                if self.use_GAN_loss:
                        log_str += 'GAN_D:{:.2e} '.format(loss_D["GAN_loss_D"].mean())
